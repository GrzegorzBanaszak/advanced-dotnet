...
namespace ClubMembershipApplication.FieldValidators
{
    public class UserRegistrationValidator : IFieldValidator
    {
        ...
        public void InitialiseValidatorDelegates()
        {
            ...
            _requiredValidDel = CommonFieldValidatorFuncions.RequireFieldValidDel;
            _stringLengValiddel = CommonFieldValidatorFuncions.StringFieldLengthValidDel;
            _dateValidDel = CommonFieldValidatorFuncions.DateFieldValidDel;
            _patternMatchValidDel = CommonFieldValidatorFuncions.FieldPatterValidDel;
            _compareFieldsValidDel = CommonFieldValidatorFuncions.FieldComparisonValidDel;
        }
        ...
        private bool ValidField(int fieldIndex, string fieldValue, string[] fieldArray, out string fieldInvalidMessage)
        {
            ...
            switch (userRegistrationField)
            {
                case FieldConstants.UserRegistrationField.EmailAddress:
                    ...
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPatterns.Email_Address_RegEx_Pattern) ? $"You must enter a valid email address:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage);
                    break;
                case FieldConstants.UserRegistrationField.FirstName:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_stringLengValiddel(fieldValue, FirstName_Min_Length, FirstName_Max_Length) ? $"You first name must be between {FirstName_Min_Length} - {FirstName_Max_Length}:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage);
                    break;
                case FieldConstants.UserRegistrationField.LastName:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_stringLengValiddel(fieldValue, LastName_Min_Length, LastName_Max_Length) ? $"You last name must be between {LastName_Min_Length} - {LastName_Max_Length}:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : fieldInvalidMessage);
                    break;
